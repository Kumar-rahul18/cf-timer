  ...
  function startTimer(duration, display) {
    ...
    var intervalId = setInterval(function() {
      if (document.getElementsByClassName("verdict-accepted").length) {
        saveEndStateAndClearInterval(intervalId, location.href, display, "Finished");
      }
      ...
      if (--timer < 0) {
        saveEndStateAndClearInterval(intervalId, location.href, display, "Timed Out");
      } else {
  ...
  chrome.storage.local.get(location.href, function(obj) {
    if (!obj[location.href]) {
      ...
      if (duration !== "Finished" && duration !== "Timed Out") {
        ...
        var seconds = parseInt((duration % 3600) % 60, 10);
        hours = hours < 10 ? "0" + hours : hours;
        minutes = minutes < 10 ? "0" + minutes : minutes;
        seconds = seconds < 10 ? "0" + seconds : seconds;
        if (!hours) {
